...
impl VirtualProject {
    ...
    pub fn from_files(files: HashMap<PathBuf, String>) -> Self {
        ...
        for (path, source) in files {
            let table = crate::symbols::build_symbol_table(&source);
            for name in crate::symbols::extract_rule_registrations(&source) {
                registered_rules
                    .entry(name.clone())
                    .or_insert_with(|| make_rule_symbol(&name));
            }
    ...
    pub fn load_from_workspace(
        ...
        tracing::debug!("semantic::load_from_workspace: walking {} visible files", visible.len());
        for (rel, path) in visible {
            ...
            let symbols = match crate::cache::load_or_parse_symbols(&path, &rel, cache_dir) {
                Ok(s) => s,
                Err(e) => {
                    tracing::warn!("load_or_parse_symbols failed for {} ({}); skipping", path.display(), e);
                    continue;
                }
            };
...
const BUILTIN_CALLEES: &[&str] = &["vector", "xsVectorSet", "xsVectorGetX", "xsVectorGetY", "xsVectorGetZ"];
...
fn is_callable_symbol(s: &Symbol) -> bool { matches!(s.kind, SymbolKind::Function | SymbolKind::Rule) }
...
pub fn resolve_callee<'a>(
    ...
    if let Some(m) = merged {
        if let Some(ms) = m.find(name) {
            if is_callable_symbol(&ms.symbol) && (ms.symbol.is_mutable || ms.effective_line() < call_line) {
...
impl SemanticChecker {
    pub fn new(project: VirtualProject) -> Self { Self { project } }
...
pub fn check_all(project: &VirtualProject, engine: &EngineApi, current_file: &Path) -> DiagnosticsByUri {
    ...
    if let Some(mv) = merged.as_ref() {
        ...
        merge_diagnostics_under_uri(&mut out, current_uri, check_mutable_redefinitions(project));
    }
...
fn merge_diagnostics_under_uri(base: &mut DiagnosticsByUri, uri: Option<Uri>, diags: Vec<Diagnostic>) {
...
fn build_merged_view_from_project(project: &VirtualProject, current_file: &Path) -> Option<MergedView> {
...
/// them.
fn effective_line(ms: &crate::merged_view::MergedSymbol) -> u32 { ms.effective_line() }
...
pub fn check_extern_collisions(
    ...
    for path in paths_in_scope {
        ...
        for sym in &file.table.symbols {
            by_name.entry(sym.name.clone()).or_default().push((path, sym));
        }
    }
    ...
    if let Some(m) = merged {
        ...
        for (uri, ds) in diags {
            if uri == current_uri || m.files().any(|p| Uri::from_file_path(p) == Some(uri.clone())) {
...
fn emit_extern_collision(
    ...
    let Some(uri) = Uri::from_file_path(decl_path) else {
        return;
    };
...
pub fn check_forward_declarations(
    ...
    for (callee, callee_range) in calls {
        ...
        let resolved_anywhere = project.files.values().any(|f| f.table.find(&callee).is_some());
        ...
        let message = if resolved_anywhere {
            ...
            format!("Error 0310: invalid symbol lookup '{}' at line {}", callee, callee_range.start.line + 1)
        };
...
fn forward_callable(project: &VirtualProject, current_file: &Path, callee: &str, call_line: u32) -> bool {
...
mod tests {
    ...
    use crate::{
        ...
        merged_view::{MergedView, VisibilityProvenance},
        workspace::{VirtualProject as WorkspaceVirtualProject, Workspace},
    };
    ...
    fn project(files: &[(&str, &str)]) -> VirtualProject {
        let map: HashMap<PathBuf, String> = files.iter().map(|(path, src)| (p(path), src.to_string())).collect();
    ...
    fn engine_api() -> &'static EngineApi {
        ...
        ENGINE.get_or_init(|| {
            ...
            EngineApi::load_from_archive(&archive, &cache_dir).expect("load engine API from Doxygen archive")
        })
    }
    ...
    fn test_resolve_callee_finds_engine_api() {
        ...
        assert!(matches!(res, Resolution::Engine(_)), "expected engine API resolution, got {res:?}");
    }
    ...
    fn test_resolve_callee_prefers_workspace_over_engine_api() {
        ...
        let res = resolve_callee(&prj, engine_api(), None, "kbUnitGetPosition", 10);
        assert!(matches!(res, Resolution::Workspace(_)), "expected workspace shadow of engine API, got {res:?}");
    }
    ...
    fn test_resolve_callee_returns_none_for_truly_unknown() {
        ...
        assert!(matches!(res, Resolution::Unresolved), "expected unresolved for unknown callee, got {res:?}");
    }
    ...
    fn test_resolve_callee_finds_registered_rule() {
        let prj = project(&[("a.xs", "void init() { xsEnableRule(\"updateBreakdown\"); }\n")]);
        let res = resolve_callee(&prj, engine_api(), None, "updateBreakdown", 10);
        match res {
            Resolution::Workspace(sym) => assert_eq!(sym.kind, SymbolKind::Rule),
    ...
    fn test_resolve_callee_unknown_rule_emits_error() {
        ...
        assert!(
            diags
                .iter()
                .any(|d| { d.message.contains("updateBreakdown") && d.message.contains("Error 0310") }),
    ...
    fn merged_fixture(files: &[(&str, &str)], current_rel: &str) -> (TempDir, VirtualProject, MergedView, PathBuf) {
    ...
    fn total_count(diags: &DiagnosticsByUri) -> usize { diags.values().map(|v| v.len()).sum() }
    ...
    fn extern_collision_fires_when_current_file_has_no_includes() {
        ...
        assert!(
            ...
            "expected at least one extern collision diagnostic; a.xs declares extern gFoo, b.xs declares non-extern \
             gFoo, neither includes the other — the LSP must see this"
        );
        assert!(
            diags
                .values()
    ...
    fn mutable_redefinition_across_include_is_checked() {
        ...
        let (_tmp, _prj, merged, _current) = merged_fixture(
            &[
                ("ai/a.xs", "mutable void helper(int x = 1) {}\ninclude \"b.xs\";\n"),
    ...
    fn direct_include_resolves_symbol() {
        ...
        let (_tmp, prj, merged, current) = merged_fixture(
            &[
                ("ai/include_forward_decl_ok.xs", include_str!("semantic_fixtures/include_forward_decl_ok.xs")),
                ("ai/include_helper.xs", include_str!("semantic_fixtures/include_helper.xs")),
            ],
    ...
    fn transitive_include_resolves_symbol() {
        ...
        let (_tmp, prj, merged, current) = merged_fixture(
            &[
                ("ai/transitive.xs", "include \"include_forward_decl_ok.xs\";\nvoid caller() { helper(); }\n"),
                ("ai/include_forward_decl_ok.xs", include_str!("semantic_fixtures/include_forward_decl_ok.xs")),
                ("ai/include_helper.xs", include_str!("semantic_fixtures/include_helper.xs")),
            ],
            "ai/transitive.xs",
        );
        let ms = merged.find("helper").expect("helper should be visible transitively");
        assert!(
            matches!(ms.provenance, VisibilityProvenance::TransitiveInclude { depth: 2, .. }),
    ...
    fn cycle_includes_do_not_loop() {
        ...
        let (_tmp, _prj, merged, _current) = merged_fixture(
            &[
                ("ai/include_cycle_a.xs", include_str!("semantic_fixtures/include_cycle_a.xs")),
                ("ai/include_cycle_b.xs", include_str!("semantic_fixtures/include_cycle_b.xs")),
            ],
        ...
        let a_count = merged.symbols().iter().filter(|ms| ms.symbol.name == "aFn").count();
        let b_count = merged.symbols().iter().filter(|ms| ms.symbol.name == "bFn").count();
        assert_eq!(a_count, 1);
        assert_eq!(b_count, 1);
    }
    ...
    fn missing_include_target_produces_diagnostic() {
        ...
        let (_tmp, _prj, merged, _current) = merged_fixture(
            &[
                ("ai/include_missing.xs", include_str!("semantic_fixtures/include_missing.xs")),
                ("ai/include_helper.xs", include_str!("semantic_fixtures/include_helper.xs")),
            ],
            "ai/include_missing.xs",
        );
        let missing_targets: Vec<_> = merged.missing_includes().iter().map(|d| d.target.clone()).collect();
        assert!(
    ...
    fn static_symbol_in_included_file_is_hidden() {
        ...
        let (_tmp, _prj, merged, _current) = merged_fixture(
            &[
                ("ai/include_static_hidden.xs", include_str!("semantic_fixtures/include_static_hidden.xs")),
                ("ai/include_static_helper.xs", include_str!("semantic_fixtures/include_static_helper.xs")),
            ],
            "ai/include_static_hidden.xs",
        );
        assert!(merged.find("gHidden").is_none(), "static variable from included file should be hidden");
    }
    ...
    fn load_from_workspace_skips_bad_files() {
        ...
        let good_table = semantic_project
            ...
            .table
            .clone();
        assert!(good_table.find("liveHelper").is_some(), "good file's symbols must be present in the project");
