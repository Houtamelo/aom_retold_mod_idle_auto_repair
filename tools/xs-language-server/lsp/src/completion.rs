...
use tower_lsp_server::ls_types::{CompletionItem, CompletionItemKind, CompletionParams};
...
fn aiplan_item(c: &crate::engine_api::AiplanConstant) -> CompletionItem {
    CompletionItem {
        ...
        detail: Some(format!("{} {} = {}", c.variable_type, c.name, c.variable_value)),
        documentation: Some(tower_lsp_server::ls_types::Documentation::MarkupContent(
            tower_lsp_server::ls_types::MarkupContent {
                kind:  tower_lsp_server::ls_types::MarkupKind::Markdown,
...
mod tests {
    ...
    fn api() -> EngineApi { EngineApi::default() }
        ...
        CompletionParams {
            text_document_position: tower_lsp_server::ls_types::TextDocumentPositionParams {
                text_document: TextDocumentIdentifier {
                    uri: tower_lsp_server::ls_types::Uri::from_str("file:///tmp/test.xs").unwrap(),
                },
    ...
    fn included_file_symbols_are_offered() {
        ...
        let merged = build_merged_view("ai/a.xs", "include \"b.xs\";\n", &[("ai/b.xs", "void included() {}\n")]);
        let items = complete(&api(), Some(&merged), "", &params_at(1, 0));
        let labels = labels(&items);
        assert!(labels.contains(&"included"), "included function missing: {:?}", labels);
    }
    ...
    fn included_visibility_filters_static_and_keeps_extern() {
        ...
        assert!(!labels.contains(&"gHidden"), "static variable from include should be hidden");
    }

    #[test]
    fn prefix_filters_included_symbols() {
        let merged =
            build_merged_view("ai/a.xs", "include \"b.xs\";\n", &[("ai/b.xs", "void alpha() {}\nvoid beta() {}\n")]);
