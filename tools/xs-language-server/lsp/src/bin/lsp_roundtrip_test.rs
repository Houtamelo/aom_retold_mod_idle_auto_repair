...
mod wire_helpers {
    ...
    pub fn read_framed_json_message<R: Read>(reader: &mut R) -> std::io::Result<Value> {
        ...
        let header_end = loop {
            ...
            if n == 0 {
                return Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "EOF before headers complete"));
            }
    ...
    fn classify_message(value: Value) -> Option<LspMessage> {
        ...
        match (id, method) {
            ...
            (Some(id), None) => {
                ...
                Some(LspMessage::Response { id, result_or_error })
            }
    ...
    pub fn find_notifications<'a>(
        ...
        messages
            .iter()
            .filter(move |m| matches!(m, LspMessage::Notification { method: mthd, .. } if mthd == method))
    }
}
...
fn read_response_with_id<R: Read>(reader: &mut R, expected_id: i64) -> std::io::Result<serde_json::Value> {
    loop {
...
fn run_baseline() -> bool {
    ...
    let mut child = Command::new(&server_path)
        ...
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::from(std::fs::File::create("/tmp/xs_lsp_server_stderr.log").expect("create stderr log")))
        .spawn()
    ...
    loop {
        match read_framed_message(&mut cursor) {
            Ok(msg) => {
                eprintln!("[test] parsed msg: id={:?}, method={:?}", msg.get("id"), msg.get("method"));
                if let Some(id) = msg.get("id").and_then(|v| v.as_i64()) {
    ...
    eprintln!(
        "[test] init_resp: {}, shutdown_resp: {}, completion_resp: {}, hover_resp: {}, definition_resp: {}, \
         hover_workspace: {}, definition_workspace: {}, hover_constant: {}, document_symbol: {}, references: {}, \
         rename: {}, prepare_rename: {}, prepare_rename_engine: {}",
        init_resp.is_some(),
    ...
    if has_hover_name && has_hover_signature && has_hover_type {
        ...
        println!(
            "FAIL: hover did not return expected content (name={has_hover_name}, fence={has_hover_signature}, \
             type={has_hover_type})"
        );
    ...
    if is_null && no_stub_uri {
        ...
        println!(
            "FAIL: definition did not return null for engine-API symbol aiEcho (is_null={is_null}, \
             no_stub={no_stub_uri})"
        );
    ...
    let has_workspace_detail =
        hover_workspace_raw.contains("int helper") || hover_workspace_raw.contains("helper(int a, int b)");
    if has_workspace_name && has_workspace_kind && has_workspace_detail {
        ...
        println!(
            "FAIL: hover workspace did not return expected content (name={has_workspace_name}, \
             kind={has_workspace_kind}, detail={has_workspace_detail})"
        );
    ...
    let has_correct_line = def_workspace_raw.contains("\"line\":7") || def_workspace_raw.contains("\"line\": 7");
    if has_real_file && no_stub_uri && has_correct_line {
        println!("PASS: definition returned real file:line for workspace function");
    } else {
        println!(
            "FAIL: definition workspace did not return real location (file={has_real_file}, no_stub={no_stub_uri}, \
             line={has_correct_line})"
        );
    ...
    if has_constant_name && has_constant_kind && has_constant_detail {
        ...
        println!(
            "FAIL: hover constant did not return expected content (name={has_constant_name}, \
             kind={has_constant_kind}, detail={has_constant_detail})"
        );
    ...
    if sym_count_rule >= 1 && sym_count_func >= 1 && sym_count_const >= 1 {
        println!(
            "PASS: document symbol returned 3 symbols (rule={sym_count_rule}, function={sym_count_func}, \
             constant={sym_count_const})"
        );
    } else {
        println!(
            "FAIL: document symbol did not return all 3 expected symbols (rule={sym_count_rule}, \
             func={sym_count_func}, const={sym_count_const})"
        );
    ...
    if rename_edits_count >= 3 && has_changes_field {
        println!(
            "PASS: rename returned WorkspaceEdit with 3+ edits (count={rename_edits_count}, \
             has_changes={has_changes_field})"
        );
    } else {
        println!(
            "FAIL: rename did not return expected WorkspaceEdit (count={rename_edits_count}, \
             has_changes={has_changes_field})"
        );
    ...
    let is_not_null =
        !prepare_rename_raw.contains("\"result\":null") && !prepare_rename_raw.contains("\"result\": null");
    ...
    let has_id_13 = prepare_rename_engine_raw.contains("\"id\":13") || prepare_rename_engine_raw.contains("\"id\": 13");
    let has_null_result =
        prepare_rename_engine_raw.contains("\"result\":null") || prepare_rename_engine_raw.contains("\"result\": null");
    ...
    let has_type_err = types_diag_raw.contains("expected argument 1 of type `string` for `aiEcho`, got `int`");
    ...
    if has_count_err && has_type_err && has_no_err_on_valid_calls {
        ...
        println!(
            "FAIL: typecheck incomplete (count_err={has_count_err}, type_err={has_type_err}, \
             no_false_positive={has_no_err_on_valid_calls})"
        );
...
fn run_workspace_tests() -> bool {
    ...
    std::fs::write(&mod_a_main, "include \"core/core.xs\"\n\nvoid mainAssistance() { modCore(); }\n").unwrap();
    ...
    let mod_a_uri = Uri::from_file_path(&mod_a).unwrap();
    let mod_b_uri = Uri::from_file_path(&mod_b).unwrap();
    let mod_a_main_uri = Uri::from_file_path(&mod_a_main).unwrap();
    let mod_b_file_uri = Uri::from_file_path(&mod_b_file).unwrap();
    let unowned_uri = Uri::from_file_path(&unowned).unwrap();
    let vanilla_core_uri = Uri::from_file_path(&vanilla_core).unwrap();
    ...
    let with_uri = |uri: &Uri| uri.to_string();
...
fn run_semantic_fixture(
    ...
    let file_uris: Vec<(String, Uri)> = open_files
        .iter()
        .map(|rel| {
            let path = mod_game.join(rel);
...
fn setup_include_mod(
    ...
    main_content: &str,
    util_content: &str,
) -> (std::path::PathBuf, std::path::PathBuf, Uri, Uri) {
    let pid = std::process::id();
    let tmp = std::env::temp_dir();
    ...
    let util_uri = Uri::from_file_path(&util_path).unwrap();
...
fn run_include_request_scenario(
    ...
    util_content: &str,
    request_id: i64,
    request: &dyn Fn(&Uri) -> serde_json::Value,
    check: &dyn Fn(&serde_json::Value, &Uri) -> bool,
) -> bool {
    let (game_root, mod_root, main_uri, util_uri) = setup_include_mod(name, main_content, util_content);
    ...
    let has_panic = stderr_text.lines().any(|l| l.contains("panic") || l.contains("FATAL"));
    let check_ok = response.as_ref().map(|resp| check(resp, &util_uri)).unwrap_or(false);
...
fn run_hover_across_include() -> bool {
    ...
    run_include_request_scenario(
        ...
        &|resp, util_uri| {
            ...
            raw.contains("included_fn") && raw.contains("void included_fn()") && raw.contains(util_uri.path().as_str())
        },
    )
}
...
fn run_cycle_does_not_hang() -> bool {
    ...
    let (game_root, mod_root, main_uri, _util_uri) = setup_include_mod("cycle_does_not_hang", main, util);
