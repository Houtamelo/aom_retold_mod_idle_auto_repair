...
use crate::{
    ...
    workspace::{IncludeEdge, IncludeRoot, VirtualProject, Workspace},
};
...
impl IncludeGraph {
    ...
    pub fn edges(&self) -> &[IncludeEdge] { &self.edges }
...
impl MergedView {
        ...
        for (target, range) in directives {
            ...
            Ok((to_path, edge)) => {
                ...
                Err(e) => {
                    ...
                    view.missing
                        .push(IncludeDiagnostic::missing(target, file.to_path_buf(), root, range));
                }
            }
        }
    ...
    pub fn matching(&self, prefix: &str) -> impl Iterator<Item = &MergedSymbol> {
        self.symbols.iter().filter(move |ms| ms.symbol.name.starts_with(prefix))
    }

    /// The include graph built during merge.
    pub fn graph(&self) -> &IncludeGraph { &self.graph }

    /// All include diagnostics: missing targets, unreadable targets, and
    /// any future reason a target could not be folded into scope.
    pub fn missing_includes(&self) -> &[IncludeDiagnostic] { &self.missing }
    ...
    pub fn unresolved_includes(&self) -> impl Iterator<Item = &IncludeDiagnostic> {
        self.missing.iter().filter(|d| d.kind == IncludeDiagnosticKind::Missing)
    }
    ...
    pub fn own_table(&self) -> &SymbolTable { &self.own_table }

    /// Source text for a file in the include closure, if available.
    pub fn source(&self, path: &Path) -> Option<&str> { self.sources.get(path).map(|s| s.as_str()) }
    ...
    pub fn current_file(&self) -> &Path { &self.current_file }
    ...
    pub fn visibility_line(&self, name: &str) -> Option<u32> { self.find(name).map(|ms| ms.effective_line()) }

    /// All merged symbols.
    pub fn symbols(&self) -> &[MergedSymbol] { &self.symbols }
}
...
mod tests {
    ...
    fn build_view(file: &Path, game_path: &Path, mod_path: Option<&Path>) -> MergedView {
        ...
        let project = if let Some(m) = mod_path {
            ws.register_mod(Uri::from_file_path(m).unwrap()).unwrap();
    ...
    fn direct_include_makes_symbol_visible() {
        ...
        assert!(matches!(ms.provenance, VisibilityProvenance::DirectInclude { .. }));
    }
    ...
    fn transitive_include_makes_symbol_visible() {
        ...
        assert_eq!(ms.symbol.kind, SymbolKind::Function);
        assert!(matches!(ms.provenance, VisibilityProvenance::TransitiveInclude { depth: 2, .. }));
    }
    ...
    fn include_cycle_terminates_cleanly() {
        ...
        let a_count = view.symbols().iter().filter(|ms| ms.symbol.name == "aFn").count();
        let b_count = view.symbols().iter().filter(|ms| ms.symbol.name == "bFn").count();
    ...
    fn missing_include_target_produces_diagnostic() {
        ...
        let view = build_view(&a, root, None);
        let missing: Vec<_> = view.missing_includes().iter().map(|d| d.target.clone()).collect();
    ...
    fn build_random_map_view(
        ...
        let own = crate::symbols::build_symbol_table(&source);
        let cache_dir = TempDir::new().unwrap();
    ...
    fn build_succeeds_when_include_target_is_binary_xs() {
        ...
        let unreadable_targets: Vec<&str> = view.unreadable_includes().map(|d| d.target.as_str()).collect();
        assert!(
    ...
    fn build_succeeds_when_include_target_is_missing() {
        ...
        assert!(view.find("ownFn").is_some(), "own-file symbols must be present when an include target is missing");

        let missing_targets: Vec<&str> = view.unresolved_includes().map(|d| d.target.as_str()).collect();
        assert!(
    ...
    fn walk_includes_skips_unreadable_includes() {
        ...
        assert!(view.find("aFn").is_some(), "own-file symbol must be present");
        assert!(
        ...
        let unreadable: Vec<&str> = view.unreadable_includes().map(|d| d.target.as_str()).collect();
        assert!(
