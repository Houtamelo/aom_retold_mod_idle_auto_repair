...
use xs_language_server::{cache, engine_api, server::XsLanguageServer};
...
async fn main() -> anyhow::Result<()> {
    ...
    if !archive.exists() {
        bail!(
            "Could not find doxygen_retail.7z in {game_path:?}. Provide the Age of Mythology: Retold installation \
             root (the folder containing doxygen_retail.7z and game/) via --game-path <PATH>, a positional argument, \
             or the AOMR_GAME_PATH environment variable."
        );
    }
    ...
    let (service, socket) =
        LspService::build(move |client| XsLanguageServer::new(client, Arc::clone(&engine), game_path.clone())).finish();
...
fn resolve_game_path() -> anyhow::Result<PathBuf> {
    ...
    let raw = explicit
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No game folder provided. Set AOMR_GAME_PATH environment variable, pass --game-path <PATH>, or \
                 configure in IntelliJ Settings → XS Language Server → Game Folder."
            )
        })?;
