...
async fn unfixed_did_close_holds_multiple_mutexes() {
    ...
    let _uri: Url = "file:///tmp/unfixed.xs".parse().unwrap();
...
async fn production_did_close_holds_at_most_one_mutex() {
    ...
    let symbol_tables: InstrumentedMutex<HashMap<Url, SymbolTable>> =
        InstrumentedMutex::with_counter(HashMap::new(), counter.clone());
    let merged_views: InstrumentedMutex<HashMap<Url, ()>> =
        InstrumentedMutex::with_counter(HashMap::new(), counter.clone());
    let uri: Url = "file:///tmp/production.xs".parse().unwrap();
...
async fn did_close_pattern_triangulates_with_populated_stores() {
    ...
    let uri: Url = "file:///tmp/triangulate.xs".parse().unwrap();
    initial_docs.open(uri.clone(), "void main() {}".to_string());

    let mut initial_tables = HashMap::<Url, SymbolTable>::new();
    initial_tables.insert(uri.clone(), SymbolTable::default());

    let mut initial_views = HashMap::<Url, ()>::new();
