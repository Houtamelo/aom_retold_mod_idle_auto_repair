...
impl Fixture {
    ...
    fn did_open(&mut self, uri: &Url, text: &str) {
    ...
    fn definition(&mut self, uri: &Url, line: u32, character: u32) -> serde_json::Value {
        let id = self.next_id();
...
fn uri_for(root: &Path, rel: &str) -> Url {
    Url::from_file_path(root.join("game").join(rel)).unwrap()
}
