...
## `RuleDefinition` body support

`RuleDefinition` AST extraction SHALL support non-empty rule bodies that contain statements and local declarations. The grammar's `block_item^` rule uses a `TypeTable`-gated predicate dispatch (`?is_type`) so that `RuleDefinition::from_cst` succeeds for real XS rule bodies. Empty rule bodies remain valid. See `spec-rule-body-extraction.md` for the full grammar and predicate specification.
