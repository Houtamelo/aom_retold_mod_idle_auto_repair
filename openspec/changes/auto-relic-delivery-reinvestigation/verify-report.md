...
|---|---|---|
| **`kbUnitGetPlanID` semantics** | `heroIsDeliverable()` requires `kbUnitGetPlanID(heroID) == cInvalidID`. If a manually controlled human hero ever has a non-invalid plan ID while appearing idle, all deliveries will be silently suppressed. | After a natural pickup with no manual order, observe whether the delivery banner fires within one tick of the hero becoming idle. If it never fires even after the pickup animation finishes, the guard may be too strict. |
