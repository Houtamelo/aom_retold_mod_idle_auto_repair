...
- `game/ai/human_assist/human_assist.xs` — copy of vanilla `human_assist.xs` with both `include` lines and the scout hook, plus room for future feature includes.
- `game/ai/human_assist/auto_<future>.xs` — future features will live here (no extra feature logic at bootstrap).
...
- [ ] **Deploy dry-run.** Run `scripts/deploy-mods.sh` and confirm `mods/local/Human Assist Improvements/game/ai/human_assist/` contains `auto_repair.xs`, `auto_scout.xs`, and `human_assist.xs`. The first three mods' deploy blocks should still print normally.
...
- [ ] **Rollback check.** If you need to remove this mod, delete `mod/human_assist_improvements/`, revert the fourth block in `scripts/deploy-mods.sh`, revert the top-level README additions, and delete `thumbnail_human-assist-improvements.png`.
...
2. Re-add the two include lines right after the existing `include` directives:
   ```xs
...
`auto_repair.xs` and `auto_scout.xs` continue to be maintained in their canonical sibling directories.
