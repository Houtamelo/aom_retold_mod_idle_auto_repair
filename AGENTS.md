...
3. `cargo test` for unit tests (220 tests as of 2026-06-29; +5 from `tests/r5_test_honesty_repro.rs` covering R5-F-01/02/03, +6 from `tests/symbols_cleanup_repro.rs` covering R1-F-01/03, +6 from `tests/forward_decl_repro.rs` covering Issue #2 forward-decl include chain, +5 from `tests/include_goto_definition_repro.rs` covering Issue #4 include-statement navigation). Plugin tests: 81 (5 from `XsGotoDeclarationHandlerTest` covering Issue #1 + 1 covering Issue #4 forwarding, 5 from `XsColorSettingsPageTest` covering Issue #3 + 1 covering Issue #3c identifier-under-caret, 8 from `XsSyntaxHighlighterTest` covering Issue #3b, 3 from `XsTextMateIncludeKeywordTest` covering Issue #3a).
