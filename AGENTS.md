...
3. `cargo test` for unit tests (239 tests as of 2026-06-30; +5 from `tests/r5_test_honesty_repro.rs` covering R5-F-01/02/03, +6 from `tests/symbols_cleanup_repro.rs` covering R1-F-01/03, +6 from `tests/forward_decl_repro.rs` covering Issue #2 forward-decl include chain, +5 from `tests/include_goto_definition_repro.rs` covering Issue #4 include-statement navigation, +6 from `tests/semantic_tokens_repro.rs` covering Issue #3 Bucket C LSP, +5 from `tests/class_extraction_repro.rs` and +4 from `tests/semantic_token_distinctions_repro.rs` finishing Issue #3 Bucket C). Plugin tests: 96 (5 from `XsGotoDeclarationHandlerTest` covering Issue #1 + 1 covering Issue #4 forwarding, 5 from `XsColorSettingsPageTest` covering Issue #3 + 1 covering Issue #3c identifier-under-caret, 8 from `XsSyntaxHighlighterTest` covering Issue #3b, 3 from `XsTextMateIncludeKeywordTest` covering Issue #3a, 17 from `XsSemanticTokensConverterTest` + 5 from `XsLspServerDescriptorTest` covering Issue #3 Bucket C finish, plus the remaining plugin baseline tests).
