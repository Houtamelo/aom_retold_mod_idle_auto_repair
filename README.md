...
- [`mod/intelligent_auto_scout/`](mod/intelligent_auto_scout/) — **Intelligent Auto-Scout**. Replaces the engine's auto-scout button on land scouts with an engine-driven area-exploration system. The mod selects, scores, and filters land areas while the game's own `cPlanExplore` handles pathing and waypoint sequencing; it still prioritizes coverage near your town center, coordinates across multiple scouts, and routes any herdable a scout passes near to your nearest town center. Oracles keep the same area logic with explicit LOS pausing. See the mod's [README](mod/intelligent_auto_scout/README.md) for full details.
...
- Replaced the manual frontier-walk state machine with the engine's built-in `cPlanExplore` area mode. The mod builds, scores, filters, and claims land areas, then hands the ranked list to `cExplorePlanExploreAreaIDs` for the engine to path and sweep.
- Heat-map danger areas remain filtered out before the list is sent, and park/unpark coordination is explicit during herd diversion, flee recovery, and Oracle LOS pausing.
- See the mod's [README](mod/intelligent_auto_scout/README.md) for full details.
